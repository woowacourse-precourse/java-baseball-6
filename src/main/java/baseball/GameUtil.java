...
public class GameUtil {
    ...
    public static List<Integer> inputParse(String input) {
        if(input.length() != 3) throw new IllegalArgumentException("[세자리 수] 세 자리의 숫자를 입력해 주세요.");
    ...
    public static void validateValue(int value) {
        if(value > 9 || value < 1) throw new IllegalArgumentException("[벗어난 수] 1부터 9사이의 값을 입력해 주세요.");
    }
