...
public class Baseball_Main {
    ...
    private boolean Checking_User_Ball(String inputNumberString){
        ...
        // 스트라이크와 볼을 서로 확인하는 함수 생성 하기 if-else 삭제
        if(strikes == 3){
            ...
            else if(strikes > 0 && strikes < 3){
                System.out.print(strikes + STRIKES_COUNT);
            }
            else if(ball == 0 && strikes == 0){
    ...
    private int[] changeUserInputNumber(String inputNumberString){engthInputError + "\nExpect : " + correctLength + "\nInput
        // 문자열을 저장할 변수 초기화
