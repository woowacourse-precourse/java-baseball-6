...
import camp.nextstep.edu.missionutils.Console; // 입력 API
import camp.nextstep.edu.missionutils.Randoms; // 랜덤 API

public class Application {

    public static boolean inputExpt(String num, int numLength) { // 입력값 예외 처리
        if (num.length() != 3)  return true; // 입력 받은 값의 길이가 3이 아닐 경우
    ...
    public static <List> void main(String[] args) {
        ...
        while (!exit.equals("2")) {
            ...
            if (inputExpt(num, numLength)) {
                throw new IllegalArgumentException("입력 예외 발생!");
            }
