...
public class Application {
    ...
    public static ArrayList<Integer> InputNum() {
        ...
        try {
            int temp = Integer.parseInt(playerNumString);
        } catch (NumberFormatException e){
            throw new IllegalArgumentException();
        }

        for(int i=0; i<3; i++){
            playerNum.add(Character.getNumericValue(playerNumString.charAt(i)));
        }

        return playerNum;
    }
    public static int RestartCheck(){
        ...
        try {
            int temp = Integer.parseInt(playerChooseString);
        } catch (NumberFormatException e){
            throw new IllegalArgumentException();
        }
