...
public class Application {
    ...
    public static ArrayList<Integer> randomComputerNumber(){ // 랜덤으로 3자리 수를 생성하는 rendomComputerNumber
    ...
    public static ArrayList<Integer> userInputNumber(String userInput){ // 사용자가 입력한 수를 배열에 저장하는 userInputNumber
    ...
    public static int[] isStrikeAndBallNumber(ArrayList<Integer> computerNum, ArrayList<Integer> userNum){ // 볼과 스트라이크의 개수를 확인하는 isStrikeAndBallNumber
    ...
    public static void printStrikeAndBall(int[] strikeAndBall){ // 볼과 스트라이크의 개수에 따라 출력문을 생성하는 printStrikeAndBall
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Integer> computerNumber = randomComputerNumber(); // 서로 다른 3자리 수를 랜덤으로 생성하는 randomComputerNumber 메서드
        BufferedReader reader = new BufferedReader(new InputStreamReader(System.in)); // 숫자를 입력받기 위해 BufferedReader 생성
        ...
        while(isProgress){ // isProgress가 true이면 반복, false이면 종료
            ...
            ArrayList<Integer> userNumber = userInputNumber(userInput); // 입력한 값을 배열에 저장하는 userInputNumber 메서드
            if(hasDuplicate(userNumber)) // 동일한 숫자가 존재하거나, userNumber의 개수가 3이 아니거나, 0이 존재한다면 예외 발생
                throw new IllegalArgumentException();
            int[] strikeAndBall = isStrikeAndBallNumber(computerNumber, userNumber); // 볼과 스트라이크의 개수를 배열 형태로 리턴하는 isStrikeAndBall 메서드
            printStrikeAndBall(strikeAndBall); // 볼과 스트라이크의 개수를 출력하는 printStrikeAndBall 메서드
            if(strikeAndBall[1] == 3){ // 스트라이크의 개수가 3이면
                ...
                if(reStartNumber == 1){ // 1을 입력했다면 게임을 새로 시작하므로 새로운 랜덤 3자리 수를 computerNumber에 담는다
