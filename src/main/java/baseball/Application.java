...
public class Application {
    ...
    public static List<Integer> answer_list = new ArrayList<>();
    //메인 숫자야구 함수
    public static void main(String[] args) {
        ...
        while (strike != 3 || end_check == false) {
            ...
            ball = 0;
            strike = 0;
            int[] input_split = input_number();
            strike = check_StrikeAndBall(input_split, ball, strike);
            if(strike == 3){EndOrRestart(strike);}
    ...
    //스트라이크와 볼을 카운트하는 함수
    public static int check_StrikeAndBall(int[] input_split, int ball, int strike){
        //배열 길이(3) 만큼 실행
    ...
    //스트라이크가 3개 이상일 때 재시작 / 종료 여부를 입력하는 함수
    public static void EndOrRestart(int strike){
