...
public class BaseballGame {
    private enum BaseBallType { NOTHING, BALL, STRIKE }
    ...
    private void initBallNumbers() {
        computerBallList = new ArrayList<>();
        while (computerBallList.size() < totalNumber) {
    ...
    private BaseBallType gradeOneNumber(int userNumber, int nowIndex) {
        if (computerBallList.get(nowIndex) == userNumber) {
            return BaseBallType.STRIKE;
        } else {
            for (int computerInt : computerBallList) {
    ...
    private void printResult(int ballTimes, int strikeTimes) {
        if (ballTimes > 0) {
            System.out.print(ballTimes + "볼 ");
        }
        if (strikeTimes > 0) {
            System.out.print(strikeTimes + "스트라이크");
        }
        if (ballTimes + strikeTimes == 0) {
            System.out.print("낫싱");
        }
        System.out.println();
    }
    ...
    private boolean gradeAllNumber(String userInput) {
        ...
        if (strikeTimes == totalNumber) {
            System.out.println(totalNumber + "개의 숫자를 모두 맞히셨습니다! 게임 종료");
            return true;
        }
        return false;
    }
