...
public class Output {
    ...
    public void printResult(int[] matchList) {
        if (matchList[MatchState.NOTHING.getValue()] > 0) {
            ...
            System.out.println(matchList[MatchState.BALL.getValue()] + BALL + " " +
                    matchList[MatchState.STRIKE.getValue()] + STRIKE);
        }
        else if (matchList[MatchState.BALL.getValue()] > 0) {
            System.out.println(matchList[MatchState.BALL.getValue()] + BALL);
        }
        else if (matchList[MatchState.STRIKE.getValue()] > 0) {
            System.out.println(matchList[MatchState.STRIKE.getValue()] + STRIKE);
        }
    }
