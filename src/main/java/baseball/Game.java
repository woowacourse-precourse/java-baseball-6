...
public class Game {
    ...
    private String inputRestartOrExit() {
        ...
        if(!isRightAnswer(input)) {
            throw new IllegalArgumentException(); // 1과 2의 숫자가 아니기 때문에 예외 처리를 한다
        }
        return input;
    }
    ...
    private boolean isRightAnswer(String input){
        ...
        return pattern.matcher(input).matches(); // 1과 2의 숫자가 아니 라면 false
    }
    ...
    private String makeHint (int strike, int ball){
        StringBuilder sb = new StringBuilder(); //String 길이가 가변이라면 사용한다.
        if(strike != 0){
            if(ball != 0){
    ...
    private int countBall(List<Integer> answer, List<Integer> input){
        ...
        for(int idx = 0; idx < answer.size(); idx++){
            if(!answer.get(idx).equals(input.get(idx)) && input.contains(answer.get(idx))){
                count++;
            }//정답과 입력 값이 같은 위치에 있지 않고, 입력 값안에 정답 값이 있어야 함
        }
        return count;
    }
}