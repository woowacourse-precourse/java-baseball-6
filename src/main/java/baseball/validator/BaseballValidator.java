...
public class BaseballValidator {
    ...
    public boolean validator(List<Integer> baseball){
        return sizeValidator(baseball) || duplicateValidator(baseball) || rangeValidator(baseball);
    }
    //3개의 숫자인지 검증하는 메소드
    public boolean sizeValidator(List<Integer> baseball) {
    ...
    //중복되는 숫자가 있는지 검증하는 메소드 distinct를 이용해 중복 제거
    public boolean duplicateValidator(List<Integer> baseball){
    ...
    public boolean rangeValidator(List<Integer> baseball){
