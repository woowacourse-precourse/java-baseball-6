...
public class ComputerService {
    ...
    public Computer playGame(Long id) {
        ...
        if(optionalComputer.isPresent()) return optionalComputer.get();
    ...
    public String gameResult(Long id, List<Integer> userNumbers) {
        ...
        // 0스트라이크 0볼 이면
        if(strikeCount == 0 && ballCount == 0) return NumberBaseBallGameWord.NOTHING.getKrName();
        ...
        if(ballCount != 0) sb.append(ballCount).append(NumberBaseBallGameWord.BALL.getKrName()).append(" ");
        if(strikeCount != 0) sb.append(strikeCount).append(NumberBaseBallGameWord.STRIKE.getKrName());
