...
public class BaseballGameManager {
    ...
    private static final String NOT_GAME_COMMAND = "게임을 시작하거나 종료시키는 명렁어가 아닙니다.";
        ...
        while (gameProcess != GAME_TERMINATE) {
            ...
            while(!isAllStrike) {
                ...
                if (isAllStrike) {
                    ...
                    if(restart.length() > 1) throw new IllegalArgumentException(NUMBER_RANGE_EXCEPTION.getErrorMsg());
                    gameProcess = Character.getNumericValue(restart.charAt(0));
                    if(gameProcess != GAME_TERMINATE && gameProcess != GAME_CONTINUE) throw new IllegalArgumentException(NOT_GAME_COMMAND);
                }
