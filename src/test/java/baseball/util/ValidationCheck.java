...
public class ValidationCheck {
    ...
    // 유효성 검사 : 재시작/종료 확인
    public boolean validUserCommand(String cmdStr){
    ...
    // 유효성 검사 : 숫자 자리수 확인
    public boolean validUserNumber(String cmdStr){
        int cmdInt = validateNumber(cmdStr);
        int digit = Integer.toString(cmdInt).length();
        if(digit == 3){
            return validDuplicate(cmdStr); // 중복 체크
        }
        return false;
    }

    // 유효성 검사 : 중복 숫자 검사
    private boolean validDuplicate(String cmdStr){
