...
class ResultMakerTest {
    ...
    void makeResult() {
        ...
        List<Integer> strikeBall = //3스트라이크
                resultMaker.makeResult(List.of(1,2,3),computerNumbers);
        Assertions.assertEquals(strikeBall,List.of(3,0));

        strikeBall = //낫싱
                resultMaker.makeResult(List.of(6,4,7),computerNumbers);
        Assertions.assertEquals(strikeBall,List.of(0,0));


        strikeBall = //2볼
                resultMaker.makeResult(List.of(5,1,2),computerNumbers);
        Assertions.assertEquals(strikeBall,List.of(0,2));


        strikeBall = //1스트라이크 1볼
                resultMaker.makeResult(List.of(1,3,0),computerNumbers);
