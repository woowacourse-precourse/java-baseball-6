...
- [ ] 플레이어가 입력하는 세자리 문자열에 대해 검증한다.
    - [ ] 3자리 수가 입력 되었는 지 검증한다.
    - [ ] 모든 입력이 1에서 9까지의 숫자만 입력이 되었는 지 검증한다.
...
- [ ] 의존성 주입을 활용해보자!
    - [ ] 각 게임은 GamingConsole이라는 Interface를 구현하는 구현체를 가진다.
    - [ ] 각 게임은 GamePrinter라는 Interface를 구현하는 구현체를 가진다.
    - [ ] GameRunner라는 객체를 만들어서 의존성을 주입 받을 수 있도록 만들어보자.
