...
🔹 이번 미션에서는 가장 마지막에 테스트 코드를 작성하게 되었다. 이로 인해 테스트를 작성하면서 기존의 코드를 수정해야 하는 상황이 발생했다. 이러한 상황을 해결해보기 위해서 테스트 방식을 알아봤고 이때, TDD 방식을 알게되었다. 테스트에 맞춰서 코드를 작성하는 것이 TDD 방식이라 이해했는데 나는 테스트의 역할을 기능이 온전히 작동하는지 확인하는 것이라는 생각에 TDD 방식을 사용하는 것이 맞는지 의문이 들었다. 아직 TDD에 대한 이해가 부족해서 잘못 이해하고 있는 부분일 수 있지만, 본래의 목적인 기능의 구현보다 테스트가 돌아가도록 코드를 작성해야 한다는 부분에 매몰되지는 않을까라는 생각이 들어 TDD 방식보다는 기능이 완성될 때마다 테스트 코드를 작성하는 방식으로 다음 미션을 진행해 봐야겠다고 생각했다.
...
🔹 미션의 첫 번째 커밋은 구현 기능 목록을 README에 작성하는 것이다. 평소 코드를 작성하면서 기능을 생각해왔던 나는 문제만 보고 설계를 하는 것이 익숙하지 않아 중간에 추가와 수정을 하는 경우가 있었다. 기능 목록을 처음부터 작성하고 느꼈던 점은 목록을 작성함으로써 프로젝트 전체의 흐름과 구조를 잡기 수월했다는 점이다. 다음 미션에서는 첫 커밋부터 설계를 더 신경 써서 기능 목록을 작성하고 이번 미션에서 고민했던 인터페이스의 사용과 `domain`과 `service` package 분리에 대한 부분을 명확히 할 생각이다.