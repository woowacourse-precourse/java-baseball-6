...
1. **도메인 모델 클래스**:
    ...
    - **`Game`**: 게임 상태를 나타내는 클래스 (컴퓨터 Balls, gameFinished), 컴퓨터 볼을 가진채로 플레이어 볼을 파라미터로 받아 비교하고, 게임 결과를 반환한다.
    - **`GameResult`**: 게임 결과를 나타내는 레코드 (볼, 스트라이크, 낫)
2. **서비스 레이어 클래스**:
    - **`GameService`**: 게임 관련 서비스를 다루는 클래스 (플레이어 입력받기, 플레이어 공 만들기, 플레이어 게임 종료 후 재 시작 여부 판단)
3. **컨트롤러 클래스**:
