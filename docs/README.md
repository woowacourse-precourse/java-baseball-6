...
게임을 초기화하는 단계이다.
- [X] `사용자` 정보를 초기화 한다.
- [X] `컴퓨터` 정보를 초기화 한다. `컴퓨터`는 사용자가 맞춰야 할 임의의 숫자 세 개를 가지고 있어야 한다. 
- [X] `사용자 입력` 단계로 넘어간다.
...
- [X] `사용자`가 입력한 숫자와 `컴퓨터`의 숫자을 비교한다.
- [X] 비교 결과를 출력한다.
- [X] 모두 일치하지 않은 경우 `사용자 입력` 단계로 돌아간다.
- [X] 모두 일치하는 경우 `게임 종료` 단계로 넘어간다.
...
- [X] `재시작 여부 확인` 단계로 넘어간다.

<br>

### `재시작 여부 확인`
사용자에게 재시작 여부를 확인받는 단계이다.
- [X] 게임 재시작 확인 문구를 출력한다.
- [X] `사용자`에게 게임 재시작 여부를 입력받는다.
- [X] 게임을 새로 시작 하는 경우 `게임 초기화` 단계로 돌아간다.
- [X] 게임을 종료 하는 경우 `애플리케이션 종료` 단계로 넘어간다.
...
- [X] `Random`에 대한 정보를 설정한다.
- [X] `StringConverter`에 대한 정보를 설정한다.

<br>

### `IoContext`
입출력에 대한 모든 동작을 담고 있는 클래스 이다.
- [X] 문자열 입력 기능을 제공한다.
- [X] 줄바꿈을 포함한 문자열 출력 기능을 제공한다.
- [X] 줄바꿈을 포함하지 않는 문자열 출력 기능을 제공한다.
...
- [X] 게임 초기화 기능을 제공한다.
- [X] `사용자`의 입력으로 수를 변경한다.
- [X] `컴퓨터`와 `사용자`의 값을 비교한다.
- [X] 게임 재시작 여부를 묻는다.
...
- [X] `BaseballContext`를 이용해 각 단계별 수행해야하는 숫자야구 동작을 수행한다. 
- [X] `IoContext`에서 제공하는 입출력 기능을 이용해 사용자와 상호작용 한다.
- [X]  다음 단계에 대한 정보를 제공한다.
...
- [X] 비교 결과를 `CompareResult`에 담아 반환한다.
