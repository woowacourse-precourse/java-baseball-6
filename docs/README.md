...
- [ ]  잘못된 값이 입력되었을 때 IllegalArgumentException
    - [ ]  `숫자를 입력해주세요` 입력 에서 3자리 수가 나오지 않았을 때
    - [ ]  `숫자를 입력해주세요` 입력 에서 숫자가 아닌 영어나 다른 문자가 입력되었을 때
    - [ ]  `게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요.` 입력에서 1,2 가 아닌 다른 입력이 들어왔을 때
- [x]  숫자 비교
