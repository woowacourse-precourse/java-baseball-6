...
**◼️**build.gradle 파일을 변경할 수 없고, 외부 라이브러리를 사용하지 않아야한다는 점.

**◼️**Java 코드 컨벤션 가이드를 준수하며 프로그래밍한다는 점.
...
**◼️**제한된 라이브러리: ...missionutils.Randoms와 ...missionutils.Console에서 제공하는 API를 사용하여 구현
...
여기서 눈여겨본 요구 사항은 다음과 같다. 하지만 구현전 word에 작성했고, readMe에 나중에추가했다.

◼️기능을 구현하기 전 docs/README.md에 구*현할 기능 목록을 정리해 추가*한다
