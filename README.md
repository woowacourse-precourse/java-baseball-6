...
- [ ] Input
  ...
  - [ ] 입력으로 받은 숫자 3개를 분리한다
  - [ ] 서로 다른 수 인지 확인한다
  - [ ] 사용자의 숫자가 하나가 1~9의 수 인지 검증한다
  - [ ] 잘못된 입력이 주어질 경우 IllegalArgumentException을 발생시킨 후 애플리케이션은 종료한다
- [ ] Core
  - [ ] 진행 조건
    - [ ] 컴퓨터는 중복없이 랜덤한 3자리 숫자를 생성한다
    - [ ] 숫자가 같은 자리에서 하나가 일치하면 스트라이크
    - [ ] 숫자가 다른 자리에서 하나가 일치하면 볼
    - [ ] 숫자가 전혀 일치하지 않으면 낫싱
    - [ ] 배열 사이즈 1짜리 판단해보기
    - [ ] 배열 사이즈 2짜리 판단해보기
    - [ ] 배열 사이즈 3짜리 판단해보기
  - [ ] 종료 조건
      - [ ] 3개의 숫자를 모두 맞히면 해당 round가 종료된다
  - [ ] 하나의 round종료시 게임을 새로 시작하려면 1, 종료하려면 2를 입력
- [ ] Output
  - [ ] 게임 시작 문구 출력 
  - [ ] 볼, 스트라이크, 낫싱을 출력한다
- [ ] 추가사항
