...
## 구현할 기능 목록
- 게임 시작 
  - 정답 숫자 랜덤 생성 (1~9, 중복 없는 세 자리)
- 게임 진행 
  - 사용자 입력 (1~9, 중복 없는 세 자리) 체크 
  - 스트라이크, 볼 체크 및 힌트 로직
  - 게임 종료 또는 재시작 로직
- 각종 에러 체크
  - 사용자의 숫자야구 예상 값 입력 체크 
  - 사용자의 재진행 여부 입력 값 체크  

## 구현할 클래스 및 내용  
- Player
  - **게임을 하는 Class**
  - 사용자의 입력 (숫자야구 예상 값) 을 받음
- Computer
  - **정답을 알려주는 Class**
  - 랜덤한 세 자리의 숫자를 만듦
- BaseballGameStatus
  - **매 판의 ball, strike 및 다음 게임 진행 여부를 체크하는 Class** 
...
- BaseballGameView
  ...
  - 게임의 진행 안내 문구를 출력하도록 함 
    ...
    - 진행 문구 - "숫자를 입력해주세요"
    - 결과 문구 - "X볼 X스트라이크" or "X개의 숫자를 모두 맞히셨습니다! 게임 종료"
    - 종료 문구 - "게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요." 
