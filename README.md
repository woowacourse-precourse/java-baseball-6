...
- 컴퓨터는 1부터 9까지의 서로 다른 임의의 숫자 3개를 선택한다.
...
- 사용자로부터 3자리의 서로 다른 숫자를 입력받는다.
  ...
  - 정상적인 값을 입력한 경우, 다음 과정에 따라 결과를 계산한다.
    - 사용자가 입력한 숫자와 컴퓨터가 선택한 숫자를 비교한다.
    - 스트라이크와 볼 개수를 출력한다.
    - 3스트라이크가 될 때까지 게임을 반복한다.
    - 3스트라이크가 되면 "3개의 숫자를 모두 맞히셨습니다! 게임 종료"를 출력한다.

### 문제 풀이 이후

- "게임을 새로 시작하려면 1, 종료하려면 2를 입력하세요." 문구를 출력한다.
- 사용자의 입력에 따라 재시작 분기를 시작한다.